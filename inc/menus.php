 ...
 * @return string
 */
add_filter('nav_menu_item_title', 'filter_vm_nav_menu_item_title', 10, 4);
function filter_vm_nav_menu_item_title($title, $item, $args, $depth)
{
 ...
 * @return array
 */
add_filter('nav_menu_css_class', 'filter_vm_nav_menu_css_class', 15, 3);
function filter_vm_nav_menu_css_class($classes, $item, $args)
{
	if ($args->theme_location === 'primary-menu') {
		$icon = get_field('icon', $item);
		if ($icon) {
